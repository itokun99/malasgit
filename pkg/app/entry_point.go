...
import (
	...
	"github.com/itokun99/malasgit/pkg/logs/tail"
...
func Start(buildInfo *BuildInfo, integrationTest integrationTypes.IntegrationTest) {
	...
	tempDir, err := os.MkdirTemp(tempDirBase, "malasgit-*")
	if err != nil {
	...
	appConfig, err := config.NewAppConfig("malasgit", buildInfo.Version, buildInfo.Commit, buildInfo.Date, buildInfo.BuildSource, cliArgs.Debug, tempDir)
	if err != nil {
		log.Fatal(err.Error())
	}
...
func parseCliArgsAndEnvVars() *cliArgs {
	...
	gitArg := ""
	flaggy.AddPositionalValue(&gitArg, "git-arg", 1, false, "Panel to focus upon opening malasgit. Accepted values (based on git terminology): status, branch, log, stash. Ignored if --filter arg is passed.")
	...
	tailLogs := false
	flaggy.Bool(&tailLogs, "l", "logs", "Tail malasgit logs (intended to be used when `malasgit --debug` is called in a separate terminal tab)")
...
func parseGitArg(gitArg string) appTypes.GitArg {
	...
	log.Fatalf("Invalid git arg value: '%s'. Must be one of the following values: %s. e.g. 'malasgit status'. See 'malasgit --help'.",
		gitArg,
...
// the buildInfo struct we get passed in is based on what's baked into the malasgit
// binary via the LDFLAGS argument. Some malasgit distributions will make use of these
...
func mergeBuildInfo(buildInfo *BuildInfo) {
	...
	if ok {
		...
		// if malasgit was built from source we'll show the version as the
...
func getTempDirBase() string {
	...
	tmpDirBase := filepath.Join(tempDir, "malasgit-"+user.Uid)
