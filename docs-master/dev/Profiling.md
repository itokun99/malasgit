...
malasgit with the `-profile` command line flag. This tells it to start an
...
While malasgit is running with the `-profile` flag, perform a CPU profile and
...
it gives you 20 seconds to perform the action in malasgit that you are interested
in measuring.
